...
class MeshDriver : public OpenGLViewer
{
	...
	std::string obj_mesh_name = "jetFighter.obj"; ////obj file name
...
public:
	...
	void Load_Mesh(std::string name)
	{
		if (use_obj_mesh)
		{
			...
			Obj::Read_From_Obj_File(name, meshes);
			*tri_mesh = *meshes[0];
	...
	{
		Create_Background(OpenGLColor(0.1f, 0.1f, 0.17f, 1.f), OpenGLColor(0.4f, 0.4f, 0.4f, 1.f));
		////initialize tri mesh
		opengl_tri_mesh = Add_Interactive_Object<OpenGLTriangleMesh>();
		tri_mesh = &opengl_tri_mesh->mesh;

		Load_Mesh(obj_mesh_name);
	...
	virtual void Initialize_Common_Callback_Keys()
	{
		...
		Bind_Callback_Key('z', &Keyboard_Event_Toggle_Use_Obj_Mesh_Func, "Toggle Use Obj Mesh");
		Bind_Callback_Key('x', &Keyboard_Event_Toggle_Obj_Cap_Func, "Toggle Cap");
		Bind_Callback_Key('c', &Keyboard_Event_Toggle_Obj_JetFighter_Func, "Toggle JetFighter");
		Bind_Callback_Key('v', &Keyboard_Event_Toggle_Obj_Bunny_Func, "Toggle Bunny");
	}

	virtual void Keyboard_Event_Subdivision()
	{
		std::cout << "Loop subdivision iteration " << ++iteration << std::endl;
	...
	virtual void Keyboard_Event_Toggle_Obj_Cap()
	{
		if (!use_obj_mesh || obj_mesh_name != "cap.obj")
		{
			use_obj_mesh = true;
			obj_mesh_name = "cap.obj";
			Keyboard_Event_Restore();
		}
	}
	Define_Function_Object(MeshDriver, Keyboard_Event_Toggle_Obj_Cap);
	// toggle jetFighter
	virtual void Keyboard_Event_Toggle_Obj_JetFighter()
	{
		if (!use_obj_mesh || obj_mesh_name != "jetFighter.obj")
		{
			use_obj_mesh = true;
			obj_mesh_name = "jetFighter.obj";
			Keyboard_Event_Restore();
		}
	}
	Define_Function_Object(MeshDriver, Keyboard_Event_Toggle_Obj_JetFighter);
	// toggle bunny
	virtual void Keyboard_Event_Toggle_Obj_Bunny()
	{
		if (!use_obj_mesh || obj_mesh_name != "my_mesh.obj")
		{
			std::ifstream file("my_mesh.obj");
			if (!file.good())
			{
				use_obj_mesh = false;
				std::cout << "my_mesh.obj not found, please put it in the same directory as the executable" << std::endl;
				Keyboard_Event_Restore();
				return;
			}

			use_obj_mesh = true;
			obj_mesh_name = "my_mesh.obj";
			Keyboard_Event_Restore();
		}
	}
	Define_Function_Object(MeshDriver, Keyboard_Event_Toggle_Obj_Bunny);
	// toggle use obj mesh
	virtual void Keyboard_Event_Toggle_Use_Obj_Mesh()
	{
		if (use_obj_mesh)
		{
			use_obj_mesh = false;
	...
	Define_Function_Object(MeshDriver, Keyboard_Event_Toggle_Use_Obj_Mesh);
