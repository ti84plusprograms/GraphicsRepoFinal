...
vec4 shading_normal() 
{
    ...
    float r = (_normal.x + 1.f) / 2.f;
    float g = (_normal.y + 1.f) / 2.f;
    float b = (_normal.z + 1.f) / 2.f;
...
{
    /* your implementation starts */
    vec3 l_ambient = ka * light.Ia;
    
    return vec4(l_ambient, 1.f);
    /* your implementation ends */
}
...
void main() 
{
    ...
    frag_color = shading_phong(light1, e, p, s1, n);
    frag_color += shading_phong(light2, e, p, light2.position, n);
