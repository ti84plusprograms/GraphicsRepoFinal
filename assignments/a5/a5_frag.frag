...
vec4 shading_texture_with_checkerboard() 
{
    ...
    float uvx = floor(uv.x * cell_size);
    float uvy = floor(uv.y * cell_size);

    if (mod(uvx + uvy, 2.0) == 0.0) {
        color = vec3(1.0);
    } else {
        color = vec3(0.0);
    }
...
vec4 shading_texture_with_phong(Light light, vec3 e, vec3 p, vec3 s, vec3 n) 
{
    ...
    vec3 l_lambertian = ka * light.Ia * tex_color + kd * light.Id * max(0.f, dot(normalize(n), l)) * tex_color;
    vec3 l_phong = l_lambertian + ks * light.Is * pow(max(0.f, dot(normalize(n), r)), shininess) * tex_color;

    color = vec4(l_phong, 1.0);
...
vec3 calc_bitangent(vec3 N, vec3 T) 
{
    ...
    /* your implementation starts */
    vec3 crossvec = cross(T, N);
    B = normalize(crossvec);
...
vec3 read_normal_texture() 
{
    ...
    /* your implementation starts */
    normal = (texture(tex_normal, vtx_uv).rgb) * 2.0 - 1.0;
...
vec4 shading_texture_with_normal_mapping()
{
    ...
    perturbed_normal = calc_perturbed_normal(TBN, read_normal_texture());
