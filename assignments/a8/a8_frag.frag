...
#define NUM_STAR 100.            /* number of stars on the sky */
#define NUM_EMISSION 10.        /* number of emission particles */
#define NUM_FIREWORKS 10        /* number of fireworks */
#define DURATION 2.             /* duration of each fireworks period */
...
vec2 starPoint(float index, float arms, float radius, float innerRadius) {
    float angleStep = PI / arms;
    float angle = index * angleStep;
    
    float rad = (mod(index, 2.0) == 0.0) ? radius : innerRadius;
...
vec3 renderStars(vec2 fragPos)
{
    ...
    for(float i = 0.; i < NUM_STAR; i++){
        ...
        /* your implementation starts */
         ...
         //generate time-varying brightness
        brightness *= sin(t * 0.5 + i * 0.1) * 0.5 + 0.5;
...
vec3 simSingleFirework(vec2 fragPos, vec2 launchPos, vec2 launchVel, float t, vec3 color)
{
    ...
    if(t < emitTime){
        ...
        for(float i = 0.; i < NUM_EMISSION; i++) {
            vec2 emitVel = starPoint(i, NUM_EMISSION / 2, 0.7, 0.35);
            // vec2 radius = vec2(0.7, 0.35);
            // if (mod(i, 2.0) == 0.0) {
            //     emitVel = (starPoint(i, NUM_EMISSION / 2.0, radius.x, radius.y));  // 0.7 outer radius, 0.35 inner radius
            // } else {
            //     emitVel = (starPoint(i, NUM_EMISSION / 2.0, radius.y, radius.x));  // 0.35 outer radius, 0.7 inner radius
            // }

            /* your implementation starts */

            float brightness = .002;
            brightness *= sin(emitT * 10. + i * 0.1) * 0.5 + 0.5;
            vec3 color = hash3d(i);
